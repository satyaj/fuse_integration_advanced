...
This usecase extends the previous but we will issue a SAOP message where the password used for the user is wrong. Some modifications are required within your project to design this use case

.Spring XML Client File
* Create a new +client-wrongpassword.xml+ file within the folder +src/test/resources/org/jboss/fuse/security/wssecuritypolicy+ using the file what you have created within the previous use case
