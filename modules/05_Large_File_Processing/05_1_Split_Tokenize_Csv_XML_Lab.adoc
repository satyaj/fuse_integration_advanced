...
The goal of this use case is to design a Junit Test case using the class +org.jboss.fuse.largefile.kokenize.SplitJavaCollectionTest.java+ and to develop 3 methods where you will cover the
following use cases :
...
For this project, you will create another Java class +org.jboss.fuse.largefile.kokenize.SplitUsingDelimiterTest.java+ where you will validate that a String containing a special character or token can be splitted.
...
. Repeat the previous use case using the method +testUsingTokenPairs+. So create a String named +record+ which contains your list of names but where each name is encapsulated with a
  start token "[" and end token "]". Define the syntax of the Apache Camel Route accordingly to process the content and split it using the Token pairs.
